...
if __name__ == "__main__":
    ...
    with open(f"./data/drugbank/drugbank.txt","r") as f:
        data_list = f.read().strip().split( '\n')

    #data_list(drug_id, protein_id, COMPOUND_SMILES,PROTEIN_SEQUENCE,label)
    data_list = [d for d in data_list if '.' not in d.strip().split()[0]]
    N = len(data_list)

    id2smi, smi2id, smi_embed = np.load('./data/drugbank/pretrain_embed/smi2vec.npy', allow_pickle=True)
    id2prot, prot2id, pro_embed = np.load('./data/drugbank/pretrain_embed/prot2vec.npy', allow_pickle=True)

    compounds, adjacencies,proteins,interactions,smi_ids,prot_ids = [], [], [], [], [], []
    model = Word2Vec.load("./data/drugbank/pretrain_embed/word2vec_30.model")
    for no, data in enumerate(data_list):
        ...
        try:
            ...
            print('Error:', no)
            continue
    print('compounds len:', len(compounds))
    print('adjacencies len:', len(adjacencies))
    print('proteins len:', len(proteins))
    print('interactions len:', len(interactions))
    print('smi_ids len:', len(smi_ids))
    print('prot_ids len:', len(prot_ids))

    dataset = list(zip(compounds, adjacencies, proteins, interactions, smi_ids, prot_ids))
    with open(f"./data/drugbank/drugbank.pickle", "wb") as f:
